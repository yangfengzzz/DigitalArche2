...
//  DigitalArche
//
//  Created by Feng Yang on 2019/12/28.
//  Copyright © 2019 Feng Yang. All rights reserved.
//
...
TEST(VDBCode, interior) {
    ...
    grid->setGridClass(openvdb::GRID_LEVEL_SET);
    grid->setName("LevelSetSphere");
    std::cout<<grid->getAccessor().isValueOn(openvdb::Coord(3,4,6))<<"\n";
    
    auto mask = openvdb::tools::sdfInteriorMask(*grid);
    std::cout<<mask->getAccessor().isValueOn(openvdb::Coord(3,4,6))<<"\n";
    
    // Create a VDB file object.
    openvdb::io::File file("InteriorGrid.vdb");
    // Add the grid pointer to a container.
    openvdb::GridPtrVec grids;
