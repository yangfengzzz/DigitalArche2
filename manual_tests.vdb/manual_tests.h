...
#include "../src.common/array_accessor2.h"
#include "../src.common/array_accessor3.h"
#include "../src.common/triangle_mesh3.h"

#include "../src.common/particle_system_data2.h"
...
#define JET_TESTS(testSetName) \
    class testSetName##Tests : public ::testing::Test { \
     ...
     protected: \
        ...
        template <typename T> \
        void saveData( \
            const vox::ConstArrayAccessor1<T>& data, \
        ...
        template <typename T> \
        void saveData( \
            const vox::ConstArrayAccessor2<T>& data, \
            const std::string& name) { \
            std::string filename = getFullFilePath(name); \
            unsigned int dim[2] = { \
        ...
        template <typename T> \
        void saveData( \
            const vox::ConstArrayAccessor3<T>& data, \
            const std::string& name) { \
            std::string filename = getFullFilePath(name); \
            unsigned int dim[3] = { \
        ...
        void saveParticleDataXy( \
            const std::shared_ptr<vox::ParticleSystemData2>& particles, \
            unsigned int frameNum) { \
            size_t n = particles->numberOfParticles(); \
            vox::Array1<double> x(n); \
            vox::Array1<double> y(n); \
        ...
        std::string getResourceFileName(const std::string& name) { \
            return pystring::os::path::join("../resources", name); \
        } \
    }; \
